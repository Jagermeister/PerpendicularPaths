...
from model.primative import Point
...
("[S]olve - " if len((self.model.board_section.goals[self.model.goal_index]).robots) == 1 else "") + "[N]ew Game - [Q]uit\r\n""")
		...
		if len(robot_direction) == 1:
			if robot_direction == "q":
				self.model.game_state = State.gg
				return
				...
				if answer is not None:
					...
					for i, move in enumerate(answer):
						...
						for m in move:
							...
							if m[1] != 0:
								print (next(d for d in self.model.directions if d.value == m[1]), end="\t")
							else:
								print ("     ", end="\t")
						print ("")
				return
		    ...
		    if robot_direction[0] == "r":
		        ...
		        robot = self.model.robots[1]
		    elif robot_direction[0] == "y":
		        robot = self.model.robots[2]
		    elif robot_direction[0] == "g":
		        robot = self.model.robots[3]

		    if robot_direction[1] == "n":
		        direction = self.model.directions[0]
		    elif robot_direction[1] == "s":
		        direction = self.model.directions[1]
		    elif robot_direction[1] == "e":
		        direction = self.model.directions[2]
		    elif robot_direction[1] == "w":
		        direction = self.model.directions[3]
...
#			self.model.display_update()
			...
			if input ("\t[P]lay again?\r\n").lower() == "p":
				...
				self.model.game_state = State.gg
		elif self.model.game_state == State.gg:
			return False
		return True
	...
	def display (self):
		...
		for j, r in enumerate(self.model.board_section.board):
			...
			for k, c in enumerate(r):
				point = Point (k, j)
				if c & self.model.directions[3].value == self.model.directions[3].value:
					print ("|", end="")
				elif k % 8 != 0:
					print (" ", end="")
				...
				#RODO: South wall
				if c & self.model.directions[1].value == self.model.directions[1].value:
					print ("_", end="")
				else:
					print (".", end="")
				self.color_update (0)

				if (k + 1) % 8 == 0 and c & self.model.directions[2].value != self.model.directions[2].value:
